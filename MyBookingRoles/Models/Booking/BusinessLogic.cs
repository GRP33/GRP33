...
namespace MyBookingRoles.Models.Booking
{
    public class BusinessLogic
    {
        ...
        public static bool IsInWorkingHours(DateTime start, DateTime end)
        {
            ...
            // checks not Sunday or Saturday
            if (start.DayOfWeek == DayOfWeek.Saturday || start.DayOfWeek == DayOfWeek.Sunday)

            {
                return false;
            }
            TimeRange workingHours = new TimeRange(TimeTrim.Hour(start, int.Parse(db.Administrations.Find(2).Value)),
              TimeTrim.Hour(start, int.Parse(db.Administrations.Find(3).Value)));
        ...
        public static bool IsInWorkingHours(TimeBlock block)
        {
            ...
            TimeRange workingHours = new TimeRange(TimeTrim.Hour(block.Start.Date, int.Parse(db.Administrations.Find(2).Value)),
             TimeTrim.Hour(block.Start.Date, int.Parse(db.Administrations.Find(3).Value)));
            return workingHours.HasInside(block);
        }

        public static string ValidateNoAppoinmentClash(Bookings appointment)
        {
            ApplicationDbContext db = new ApplicationDbContext();
            var appointments = from a in db.Bookings.Where(x => x.ArtistID == appointment.ArtistID)
                               select a;
        ...
        public static List<SelectListItem> AvailableAppointments(int docID, DateTime date)
        {
            ...
            //TimeBlock == MyTimeBlockExtention it was changed 
            ...
            var appointments = from a in db.Bookings.Where(x => x.ArtistID == docID)
                               select a;
            bool overlaps = false;
            ...
            ItemsList.Add(new SelectListItem() { Text = "No Bookings Available", Value = "DONT" });
            return ItemsList;
        }
    }
}